...
A SignalForge surface computed from one sequence is directly comparable to a
surface computed from another — across patients, sessions, instruments, sites,
or any two sources producing ordered sequences of the same kind — without
normalization, alignment, or feature engineering. This property
...
statistics of sequence `x` within windows of size `d` — not on where in the
...
any window in any sequence to any other window in any other sequence on the
